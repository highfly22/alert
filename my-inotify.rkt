...
(define-struct inotify-event (watcher
                              mask
                              cookie
                              len
                              name)
...
(define _close (get-ffi-obj "read" libc
                            (_fun #:save-errno 'posix
                                  _int32
                                  -> (r : _int32)
...
(provide
 (contract-out
  [struct inotify-event ([watcher exact-integer?]
                         [mask exact-nonnegative-integer?]
                         [cookie exact-nonnegative-integer?]
                         [len exact-nonnegative-integer?]
                         [name (or/c false/c string?)])]
  [_inotify_init (->* () exact-integer?)]
  [_inotify_add_watch (->* (exact-integer? string? exact-nonnegative-integer?) exact-integer?)]
  [_inotify_rm_watch (->* (exact-integer? exact-nonnegative-integer?) exact-integer?)]
  [_close (->* (exact-integer?) exact-integer?)]
  [read-worker (->* (place-channel?) any)]
  [read-events (->* (exact-integer?) (listof inotify-event?))]
  [buffer-size (parameter/c (and exact-positive-integer? (>/c 32)))]))
